...
"""
#Calculate mean salary for each professor rank:
df.groupby(['price'],sort=False)[['symboling']].mean()
...
dff=df.iloc[1:5, :-1] # Leave last columns
print(dff)
...
salary['rank'] = salary['horsepower'].fillna(salary['horsepower'].value_counts().index[0])


#agg() method are useful when multiple statistics are computed per column:
salary[['horsepower','price']].agg(['min','mean', 'median', 'sum'])
